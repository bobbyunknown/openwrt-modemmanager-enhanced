...
return baseclass.extend({
    ...
    _removeEmptyStrings: function (obj) {
        ...
        if (typeof obj == 'string') {
            if (obj == this._emptyStringValue) obj = null;
    ...
    getModems: function () {
        return fs.exec_direct(this._mmcliBin, [ '-L', '-J' ]).then(L.bind(function (res) {
            ...
            modems.forEach(L.bind(function (modem) {
                ...
                if (!isNaN(index)) tasks.push(this.getModem(index));
            }, this));
    ...
    getModemSims: function (modem) {
        ...
        if (sim != null && !simSlots.includes(sim)) simSlots.push(sim);

        simSlots.forEach(L.bind(function (modem) {
            var index = this._parseIndex(modem);
            if (!isNaN(index)) tasks.push(this.getSim(index));
        }, this));
        return Promise.all(tasks);
    },
    ...
    // --- новый метод для LTE сигнала ---
    getModemSignal: function(modem) {
        var index = this._parseIndex(modem['dbus-path']);
        return fs.exec_direct(this._mmcliBin, [ '-m', index, '--signal-get', '-J' ])
            .then(L.bind(function(signal) {
