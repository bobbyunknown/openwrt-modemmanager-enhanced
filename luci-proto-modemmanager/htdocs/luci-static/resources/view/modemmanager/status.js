...
return view.extend({
            ...
            return Promise.all(modems.filter(function (modem){
                ...
                return helper.getModemSims(modem.modem).then(function (sims) {
                    ...
                    return helper.getModemLocation(modem.modem).then(function (location) {
                        ...
                        return helper.getModemSignal(modem.modem).then(function(signal) {
                            modem.signal = signal;
                            return modem;
                        });
                    });
    ...
    renderSection: function(name, table) {
        var rowNodes = table.filter(function(row) { return row[1] != null; })
            .map(function(row) {
        ...
        var tableNode = rowNodes.length == 0 
    ...
    renderContent: function(modems) {
        ...
        /* ---- REAL-TIME SIGNAL QUALITY CALC ---- */
        var calcSignalQuality = function(rsrp, rsrq, snr) {
            var v = [];

            if (rsrp != null)
                v.push(Math.min(Math.max((rsrp + 140) / 96 * 100, 0), 100));

            if (rsrq != null)
                v.push(Math.min(Math.max((rsrq + 19.5) / 16.5 * 100, 0), 100));

            if (snr != null)
                v.push(Math.min(Math.max(snr / 30 * 100, 0), 100));
        ...
        modems.forEach(L.bind(function(modem) {
            ...
            var modemSection = this.renderSection(_('Modem Info'), [
                ...
                [E('abbr', { 'title': _('International Mobile Station Equipment Identity') }, _('IMEI')), modem3gpp.imei],
            ...
            if (modem.signal && modem.signal.modem && modem.signal.modem.signal) {
                var sig = modem.signal.modem.signal;
                var lte = sig.lte || {};
                ...
                var rsrp = parseValue(lte.rsrp);
                var rsrq = parseValue(lte.rsrq);
                var snr  = parseValue(lte.snr);

                var signalQuality = calcSignalQuality(rsrp, rsrq, snr);

                networkSectionRows.push(
                    [_('RSRQ'), rsrq != null ? rsrq + ' dB' : null],
                    [_('RSRP'), rsrp != null ? rsrp + ' dBm' : null],
                    [_('S/N'),  snr  != null ? snr  + ' dB' : null],
                    [_('Signal Quality'),
                        signalQuality != null
                        ? E('div', { 'class': 'cbi-progressbar', 'title': signalQuality + '%' }, [
                            E('div', { 'style': 'width:%d%%'.format(signalQuality) })
                        ])
                        : null
                    ]
                );
            ...
            var networkSection = this.renderSection(_('Network Registration'), networkSectionRows);
            ...
            var locationSection = this.renderSection(_('Cell Location'), [
                [E('abbr', { 'title': _('Cell ID') }, 'CID'), location3gpp.cid],
                [E('abbr', { 'title': _('Location Area Code') }, 'LAC'), location3gpp.lac],
                [E('abbr', { 'title': _('Mobile Country Code') }, 'MCC'), location3gpp.mcc],
                [E('abbr', { 'title': _('Mobile Network Code') }, 'MNC'), location3gpp.mnc],
                [E('abbr', { 'title': _('Tracking Area Code') }, 'TAC'), location3gpp.tac]
            ]);

            /* ---------- SIMs ---------- */
            var simTables = modem.sims.map(function(sim) {
                var p = sim.sim.properties;
                return [
                    [_('Active'), p.active],
                    [_('Operator Name'), p['operator-name']],
                    [E('abbr', { 'title': _('Integrated Circuit Card Identifier') }, 'ICCID'), p.iccid],
                    [E('abbr', { 'title': _('International Mobile Subscriber Identity') }, 'IMSI'), p.imsi]
                ];
            });
            ...
            node.firstElementChild.appendChild(
                E('div', { 'data-tab': generic.device, 'data-tab-title': generic.device }, [
                    modemSection,
                    networkSection,
                    locationSection,
                    simSection
