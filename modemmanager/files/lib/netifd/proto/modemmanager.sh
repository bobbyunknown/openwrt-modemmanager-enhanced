...
# Find modem by device-identifier or equipment-id
mm_find_modem_by_identifier() {
	local target_identifier="$1"
	local modems modem modemstatus identifier physdev
	
	modems=$(mmcli -L 2>/dev/null | grep -o '/org/freedesktop/ModemManager1/Modem/[0-9]*')
	
	for modem in $modems; do
		modemstatus=$(mmcli --modem="${modem}" --output-keyvalue 2>/dev/null)
		
		# Try device-identifier first
		identifier=$(modemmanager_get_field "${modemstatus}" "modem.generic.device-identifier")
		if [ "${identifier}" = "${target_identifier}" ]; then
			physdev=$(modemmanager_get_field "${modemstatus}" "modem.generic.physdev")
			echo "${physdev}"
			return 0
		fi
		
		# Fallback to equipment-identifier (IMEI)
		identifier=$(modemmanager_get_field "${modemstatus}" "modem.generic.equipment-identifier")
		if [ "${identifier}" = "${target_identifier}" ]; then
			physdev=$(modemmanager_get_field "${modemstatus}" "modem.generic.physdev")
			echo "${physdev}"
			return 0
...
# Auto-update device path using ModemManager device-identifier
mm_auto_update_device_path() {
	local interface="$1"
	local old_device="$2"
	local modemstatus new_device device_id equipment_id
	
	# Try old device path first
	modemstatus=$(mmcli --modem="${old_device}" --output-keyvalue 2>/dev/null)
	
	if [ -n "${modemstatus}" ]; then
		# Path still valid, store identifiers for future use
		device_id=$(modemmanager_get_field "${modemstatus}" "modem.generic.device-identifier")
		equipment_id=$(modemmanager_get_field "${modemstatus}" "modem.generic.equipment-identifier")
		
		# Store identifiers in UCI if not exists
		[ -n "${device_id}" ] && {
			local stored_id=$(uci_get network "${interface}" device_id)
			[ -z "${stored_id}" ] && {
				uci_set network "${interface}" device_id "${device_id}"
				[ -n "${equipment_id}" ] && uci_set network "${interface}" equipment_id "${equipment_id}"
				uci_commit network
				echo "Stored device identifiers for ${interface}"
			}
		}
	...
	# Path invalid, try to find modem by stored identifier
	echo "Device path '${old_device}' not found, searching by identifier..."
	
	# Get stored identifiers from UCI
	device_id=$(uci_get network "${interface}" device_id)
	equipment_id=$(uci_get network "${interface}" equipment_id)
	
	# Try to find by device-identifier first
	if [ -n "${device_id}" ]; then
		new_device=$(mm_find_modem_by_identifier "${device_id}")
	fi
	
	# Fallback to equipment-identifier (IMEI)
	if [ -z "${new_device}" ] && [ -n "${equipment_id}" ]; then
		new_device=$(mm_find_modem_by_identifier "${equipment_id}")
	fi
	
	# Last resort: use first available modem and store its identifiers
	if [ -z "${new_device}" ]; then
		echo "No stored identifier found, using first available modem"
		modemstatus=$(mmcli -m any --output-keyvalue 2>/dev/null)
		[ -n "${modemstatus}" ] && {
			new_device=$(modemmanager_get_field "${modemstatus}" "modem.generic.physdev")
			device_id=$(modemmanager_get_field "${modemstatus}" "modem.generic.device-identifier")
			equipment_id=$(modemmanager_get_field "${modemstatus}" "modem.generic.equipment-identifier")
			
			# Store identifiers for future matching
			[ -n "${device_id}" ] && uci_set network "${interface}" device_id "${device_id}"
			[ -n "${equipment_id}" ] && uci_set network "${interface}" equipment_id "${equipment_id}"
		}
	fi
